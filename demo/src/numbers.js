// Shared "numbers" morph styling — the numericText-style roll used across the
// demo (calendar stepper, live clock, playground readout, gauge values).
//
// The roll itself ships in the package behind the `numbers` prop. Here we just
...
// of the bounce off.
export const NUMBERS_SPRING = { stiffness: 632, damping: 30 };

/**
 * Numbers morph props for a TextMorph: the packaged numbers animation plus our
 * spring. The vertical roll distance is derived from each element's font size,
 * so the same props work at any type scale.
 */
export function numbersProps() {
  return {
