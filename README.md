...
| `ticker`               | `boolean`                     | `false`                            | Use the "ticker" animation (see below) instead of the default morph.                         |
| `enterSlide`           | `number` (px)                 | —                                  | Override the ticker's vertical slide distance. Set on its own to enable the slide.           |
| `stagger`              | `number` (ms)                 | —                                  | Override the ticker's per-segment stagger. `0` enters all new segments at once.              |
...
Set `ticker` to swap the default morph for a vertical roll modeled on SwiftUI's
...
```jsx
<TextMorph ticker granularity="grapheme" ease={{ stiffness: 632, damping: 30 }}>
  {value}
</TextMorph>
```

The roll distance is derived from the element's font size, so the same `ticker`
...
<TextMorph ticker enterSlide={28} stagger={0}>{value}</TextMorph>
```
