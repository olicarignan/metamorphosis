...
/**
 * Continuous spring displacement curve (0→1) used by JS-driven (rAF) animations
 * that can't consume the CSS `linear()` easing string. Mirrors the physics used
 * to build that string so a spring eases identically whether driven by CSS or JS.
 */
export function springEasingFn(params?: SpringParams): (t: number) => number {
  const { stiffness = 100, damping = 10, mass = 1 } = params ?? {};
  const omega0 = Math.sqrt(stiffness / mass);
  const zeta = damping / (2 * Math.sqrt(stiffness * mass));
  return (t: number) => (t >= 1 ? 1 : springPosition(t, omega0, zeta));
}
