...
// Numbers defaults: the vertical roll distance as a fraction of the element's
// font size, and the per-segment stagger in milliseconds.
const NUMBERS_SLIDE_RATIO = 0.5;
const NUMBERS_STAGGER_MS = 28;
...
export class TextMorph {
  ...
  // `numbers` derives it from the element's font size; otherwise no slide.
  private resolveSlide(): number {
    const { enterSlide, numbers } = this.options;
    if (enterSlide != null) return enterSlide;
    if (!numbers) return 0;
    const fontSize = parseFloat(getComputedStyle(this.element).fontSize);
    return (Number.isFinite(fontSize) ? fontSize : 16) * NUMBERS_SLIDE_RATIO;
  }

  // Per-segment stagger in ms. An explicit `stagger` wins; otherwise `numbers`
  // applies the default stagger; otherwise none.
  private resolveStagger(): number {
    const { stagger, numbers } = this.options;
    if (stagger != null) return stagger;
    return numbers ? NUMBERS_STAGGER_MS : 0;
  }
